...
**Location:**
- Pattern: Separate `tests/` directory at repo root
- Structure mirrors source layout: `tests/pipeline/`, `tests/kg/`, `tests/api/routes/`, etc.
- Co-location not used; tests live in dedicated directory tree

**Naming:**
- Test files: `test_*.py` (e.g., `test_sync_wrapper_guard.py`, `test_strip_control_characters.py`)

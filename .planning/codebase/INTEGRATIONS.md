...
**Embedding Providers (Switchable, v1):**
...
**Reranking Services (Optional, Switchable, v1):**
...
**Embedded Stores (Databasise 2.0):**

- Cozo (CozoDB) - Graph store primitive for agnostic machine.
  - Connection: In-process, no external server
  - Implementation: `databasise/stores/graph.py` (wraps `pycozo[embedded]`)
  - Role: Executes topologically-ordered component graph (CONTRACT.md §8)

- FAISS - Vector store primitive for agnostic machine.
  - Connection: In-process file-based
  - Implementation: `databasise/stores/vector.py` (wraps `faiss-cpu`)
  - Role: Similarity search for component outputs

- SQLite - Lexical/KV store for agnostic machine (internal, not exposed to users).
  - Connection: File-based local database
  - Implementation: `databasise/stores/kv.py`, `databasise/stores/lexical.py`
  - Role: Metadata storage, artifact registry, execution traces
...
**Component Identity (Databasise 2.0):**

- Configuration Hash (D-12) - JCS canonical form (RFC 8785) via `rfc8785.dumps()`.
  - Computed over: part registry, wiring, role assignments, budget config
  - Stored in: execution traces for reproducibility (CONTRACT.md §17.F3)
  - Implementation: `databasise/identity/canon.py`, `databasise/identity/env.py`

- Instance Hash - Per-namespace or per-execution unique identifier.
  - Computed from: configuration hash + seed (deterministic)
  - Role: Keying ledger entries, trace artifacts, component state
  - Implementation: `databasise/identity/instance.py`

**LLM Provider Authentication (v1):**
...
**Execution Tracing (Databasise 2.0):**

- Trace Artifacts - JSON-serialized execution records per CONTRACT.md §17.F3.
  - Schema: `docs/system-model/rig-trace.schema.json` (frozen, validated by `jsonschema`)
  - Stored in: `databasise/ledger/` and `databasise/registry_artifact/`
  - Contents: component DAG, budget spent, mutations logged, component outputs

- Budget Tracking - Token/cost accounting during execution.
  - Implementation: `databasise/runner/budget.py`
  - Tracked metrics: LLM tokens in/out, embedding calls, reranking calls
  - Enforcement: Stops execution if budget exceeded (per CONTRACT.md §15)
...
**Agnostic Machine Execution (Databasise 2.0):**

- Async Scheduler - `asyncio.TaskGroup` (Python 3.11+) for concurrent component execution.
  - Implementation: `databasise/runner/scheduler.py`
  - Ordering: `graphlib.TopologicalSorter` for DAG-based execution
  - Concurrency: Tasks in disjoint DAG levels run in parallel within single event loop

- Execution Guards - Pre-flight validation before component execution.
  - Implementation: `databasise/runner/guards.py`
  - Checks: Budget sufficiency, schema conformance, state initialization

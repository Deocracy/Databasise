...
> **Superseded 2026-09-10 (06-09-PLAN.md), correcting a claim this document adopted at research
> time.** Every `lancedb` recommendation below is superseded: it was adopted from a factually
> wrong description of the incumbent vector store. The live, owner-locked defaults are **Cozo**
> (graph) and **Faiss** (vector) — `databasise/pyproject.toml`'s actual pins,
> `.planning/ROADMAP.md`'s own dated Phase 1 amendment to success criterion 1, and
> `.claude/CLAUDE.md`'s explicit correction all agree. LanceDB was never adopted anywhere in the
> shipped engine. This document's own LanceDB rows are corrected in place below, not deleted —
> this project's own convention is to record a supersession rather than erase the superseded
> claim; each row's own correction is inline where it was made.
...
**Superseded 2026-09-10: the `lancedb` install line below was never adopted — the shipped engine
installs `faiss-cpu`, not `lancedb`, for its vector store; see the supersession note at the top of
this document.**
...
Use `pyproject.toml` `[project.optional-dependencies]` groups (`rest`, `mcp`, `eval`) so the embeddable core (`pip install databasise`) pulls in only Cozo + LanceDB + Pydantic + igraph + jsonschema — REST and MCP surfaces stay opt-in, matching "Databasise embeddable in-process" as the base install. **Superseded 2026-09-10: the shipped embeddable core pulls in Cozo + Faiss (not LanceDB) + Pydantic + igraph + jsonschema; see the supersession note at the top of this document.**

## Alternatives Considered

| Recommended | Alternative | When to Use Alternative |
|-------------|-------------|-------------------------|
| `lancedb` | `sqlite-vec` (0.1.9, actively maintained) | If the corpus is small enough that brute-force search is fine and you want vector search living in the same SQLite file as the KV primitive — appealing for minimalism but brute-force degrades past ~1M high-dim vectors, and the eval corpus plan (public bootstrap → owner's own corpus) makes that ceiling a real near-term risk, not a hypothetical one. **Superseded 2026-09-10: `lancedb` itself was never adopted as the "Recommended" column's own baseline — see the supersession note at the top of this document** |
| `lancedb` | `chromadb` (1.5.x) | If prototyping speed matters more than scale — `pip install chromadb` + two lines is the fastest path to something working, but it's memory-resident and degrades past ~1M vectors on constrained RAM, which conflicts with "8GB RAM + SSD recommended" from v1's own deployment notes. **Superseded 2026-09-10: `lancedb` itself was never adopted as the "Recommended" column's own baseline — see the supersession note at the top of this document** |
...
|-------|-----|-------------|
| `nano-vectordb` | Stale since 2024-11-11 (0.0.4.3), thin wrapper whose functionality LanceDB now covers natively with active maintenance behind it | ~~`lancedb`~~ **Superseded 2026-09-10: the replacement that actually shipped is `faiss-cpu`, not `lancedb` — see the supersession note at the top of this document** |
...
- **Superseded 2026-09-10:** the shipped engine's vector store is Faiss, not LanceDB — substitute "Faiss's" for "LanceDB's" when applying this pattern; see the supersession note at the top of this document
...
| `lancedb` | Rust-toolchain build only if installing from source | Prefer prebuilt wheels (available for standard Linux/NixOS glibc targets) to avoid pulling a Rust toolchain into the embeddable install path. **Superseded 2026-09-10: not adopted — see the supersession note at the top of this document** |

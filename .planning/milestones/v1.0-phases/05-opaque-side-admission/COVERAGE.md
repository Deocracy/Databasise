...
| promote | in-process, REST (`POST /promote`), MCP (`promote` tool) | 07-03 | MACH-07/API-09's operator-asserted promotion. Not expressible as a §18.4 selector: a selector picks which fitted modality answers a query; `promote` writes one generation record to the append-only ledger and repoints an alias, which no selector value can express. A separate operation under Phase 4 D-03's one-entry-per-operation rule and §19.6's own precedent that a differently-effecting mutation is its own port. |
| rollback | in-process, REST (`POST /rollback`), MCP (`rollback` tool) | 07-03 | RIG §PR.2's rollback path. Not expressible as a §18.4 selector: a selector picks which fitted modality answers a query; `rollback` writes a new generation record naming an explicit prior semver target and repoints the alias to it, which no selector value can express. A separate operation under Phase 4 D-03's one-entry-per-operation rule and §19.6's own precedent that a differently-effecting mutation is its own port. |
| retire | in-process, REST (`POST /retire`), MCP (`retire` tool) | 07-03 | D-07's tombstone verb. Not expressible as a §18.4 selector: a selector picks which fitted modality answers a query; `retire` writes a tombstone record for a named generation, which no selector value can express. A separate operation under Phase 4 D-03's one-entry-per-operation rule and §19.6's own precedent that a differently-effecting mutation is its own port. |
...
| promote / rollback / retire | **DISCHARGED** | Was `OPT-OUT` in Phase 4's table ("still Phase 7's own deliverable"). Discharged by 07-01/07-02/07-03 — see the promote/rollback/retire rows above. |

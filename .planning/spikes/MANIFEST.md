...
| 008 | melodyscribe | graph-prompt-design | comparison | Given spike 003's teacher labels, when the 2B and 4B emit ops under the v0.2 grammar with eight prompt designs, then routing, Proof-pass, recall, and tokens per paragraph per design | VALIDATED (reproduced: all 274 student decodes byte-identical on the owner re-run, every metric equal; one-shot and two-step clear the 0.10 bar on the 4B, LightRAG-style wording collapses, instruction-after-section helps the 4B and hurts the 2B) | prompts, graph, few-shot |
...
| 010 | melodyscribe | pipeline-order-end-to-end | standard | Given the corpus and query sets, when the harness runs end to end with W workers over graph, SQL, and vector stores, then documents per minute, recall per retrieval mode, payload tokens, and the effect of stage order and W | PARTIAL (reproduced: every retrieval and store number equal on the owner re-run, only timings differ; end to end on 20/20 documents, one-pass equals two-pass, streamed equals batched writes, workers conflict-free at W 1 to 8; flat-bonus fusion loses to vector-only recall@3 by two queries, so vector-first ranking with graph and SQL as backoff is the recommendation) | pipeline, stores, workers |

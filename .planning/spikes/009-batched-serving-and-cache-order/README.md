...
Owner re-run (2026-09-12): `run.sh` re-run end to end under the GPU lock (the 006 reproduction shared the lock, so cells interleaved with its jobs). Every exact-count result equal (validity 4/12 and 0/12; prefix 105 tokens, instruction-first shared run 319 tokens; per-request lead match 104/105); ingest and retrieval throughput cells within 12 percent of this run. The vLLM path failed the same way (`/sbin/ldconfig`); spike 006 later found `TRITON_LIBCUDA_PATH=/run/opengl-driver/lib` bypasses it, untested here.

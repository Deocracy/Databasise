...
- [Phase 07]: 07-05: Closed G-07-1 by widening each operator verb's guard-read-through-append into one `BEGIN IMMEDIATE` span (`Ledger.transaction()`) and making `(alias, minted_version)` UNIQUE at the schema — the read-side guard alone could not hold under concurrency, and the index backstops it independently of any Python path. §18 envelope untouched.
- [Phase 07]: Gate amendment — HARD-04, HARD-01 and HARD-02 deferred out of Phase 7 to their point of first need: the operator-asserted path consumes no eval bundle, no floor and no verdict, so none is reached by anything Phase 7 builds (`07-GATE-AMENDMENT.md`). All three stay Pending.
- [Phase 07]: Adversarial re-verification found a latent Phase-1 defect the new concurrency test was the first to expose — a raw `sqlite3.OperationalError` escaping `PRAGMA journal_mode=WAL` in `Ledger.__init__` (outside any transaction span), failing the phase's own gate test ~1 run in 30. Fixed in `18a5346`.
...
- ⚠️ [Phase 07] `sqlite3.connect()` sets no explicit `timeout=` and a lock-timeout `OperationalError` is not wrapped into a `SeamRefusalError` (07-REVIEW.md WR-03 residual). Reaching it needs genuine write-lock saturation (~88 simultaneous operator calls against a ~57ms span); carried as an anti-pattern, not a Phase 7 gap.

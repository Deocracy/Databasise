...
def test_default_registry_holds_exactly_nineteen_entries():
    ...
    ``relation-lookup``, ``entity-hydrate-expand``, ``relation-hydrate-expand``) — this count grows
    again once plan 03-06 ports the remaining three base-wiring positions, per design, not a
    regression.
    """
    registry = default_registry()
    assert len(registry.keys()) == 19

...
def test_w3_taint_rule_overrides_stage_nodes_downstream_of_the_opaque_ingest():
    ...
    rule permits (only a `shared`-scope write requires `stage`); `query-side` and `assemble` both
    taint to `opaque` despite their own parts' `stage` structural_depth.
    """
    evidence = evaluate_wiring("w3-lightrag-half-decomposed")
    assert evidence.violations == ()
    depths = {row.node_id: row.effective_depth for row in evidence.rows}
    assert depths == {"ingest": "opaque", "query-side": "opaque", "assemble": "opaque"}
    modes = {row.node_id: row.execution_mode for row in evidence.rows}
    assert modes == _W3_EXECUTION_MODES
    ingest_row = next(row for row in evidence.rows if row.node_id == "ingest")
    assert ingest_row.blast_radius == "permitted"


def test_distinct_execution_mode_set_across_all_three_wirings_is_exactly_three_values():
    """`confined-unit` is unreachable from these seven registered parts: the only parts
    declaring `fs`/`self_storage` are also `kind="opaque"` and take the subprocess branch first
    in `derive_execution_mode`. Assert the exact set, not a count — the absence of
    `confined-unit` is expected, not a wrong-import signal (02-RESEARCH.md).
    """
    modes = set()
    for stem in NAMED_WIRINGS:
        evidence = evaluate_wiring(stem)
        modes.update(row.execution_mode for row in evidence.rows)
    assert modes == {"in-process", "subprocess", "long-lived-service"}
...
def test_divergence_list_names_the_taint_overridden_nodes_and_is_empty_for_w1():
    ...
    assert w3_diverged == {"query-side", "assemble"}

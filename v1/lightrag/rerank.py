...
async def voyage_rerank(
    ...
    **kwargs: Any,
    ...
    Voyage uses ``top_k`` (not ``top_n``) and returns
    ``{"data": [{"index": int, "relevance_score": float}, ...]}``.
    The trailing ``**kwargs`` absorbs server-passed cohere-only params
    (enable_chunking, max_tokens_per_doc) so they never raise TypeError.

    Args:
        query: The search query
        documents: List of strings to rerank
        model: Voyage rerank model name (default: rerank-2.5)
        top_n: Number of top results to return (mapped to ``top_k`` in the request)
        base_url: API endpoint (default: https://api.voyageai.com/v1/rerank)
        api_key: Voyage AI API key; falls back to VOYAGE_API_KEY or
            RERANK_BINDING_API_KEY env vars
        extra_body: Additional body parameters merged into the request payload
        **kwargs: Absorbs unsupported keyword arguments (e.g. cohere-only params)
    ...
    # Voyage uses top_k, not top_n — only include when the caller requests a limit
    if top_n is not None:
        payload["top_k"] = top_n
    ...
    logger.debug(
        f"Voyage rerank request: {len(documents)} documents, model: {model}"
    )
    ...
    if not isinstance(data, list):
        logger.warning(
            f"Expected 'data' to be list, got {type(data)}: {data}"
        )
        data = []

    if not data:
        logger.warning("Voyage rerank API returned empty results")
        return []
    ...
    return sorted(standardized, key=lambda x: x["relevance_score"], reverse=True)

#!/usr/bin/env bash
# usage: [MAXPAR=k] run-lanes.sh [lane numbers]  (a lane with a session id in .logs and no findings resumes that session)  — Muse Spark lanes four at a time, each in its lane folder with the brief attached,
...
$(cat "$d/lane.txt")"
  ...
  else timeout 5400 opencode run "$msg" --file="$R/BRIEF.md" --pure --auto --format json -m $M --title "melodyscribe-research3-lane-$n" > "$L" 2>&1; rc=$?
...
for attempt in 1 2; do
  [ -s REPORT.md ] && break
  timeout 5400 opencode run "$(cat INTEGRATOR-PROMPT.md)" --file="$R/BRIEF.md" --pure --auto --format json -m $M --title "melodyscribe-research3-integrator" >> .logs/integrator.session.log 2>&1

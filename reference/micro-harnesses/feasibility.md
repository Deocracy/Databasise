...
| Small model routes extracted pieces to typed stores | **Proven at 4–8B with RL**; unmeasured at 2B | Mem-α, Memory-R1 abstracts (learned store/update/retrieve decisions) | Fine-tune target. Teacher on our schema, distilled into the 2B. On legion's 16 GB card this is sequence-level distillation (LoRA SFT on teacher outputs), not full-vocabulary OPD; see `runtime-and-training-stack.md` |

...
| `spike-results.md` | Verdicts and evidence from the five overnight spikes (2026-09-12), each reproduced before acceptance, and the signal they give the build. |
| `positioning.md` | MelodyScribe against LightRAG, HippoRAG 2, GraphRAG: stage-by-stage what collapses into one pass on the index side and one stream on the query side, where cost and speed come from, where accuracy above them can come from (distillation, RLVR on rig rewards, joint representation, per-corpus adaptation). |
| `efficiency-design.md` | Efficiency design after the deep-research run: OneGen-style trained `[EMB]`/`[RQ]` tokens replace pooled spans, first-order levers (output tokens, batching, speculative decoding, quantisation), why KV modules are second-order at 2B, structure-must-earn-its-cost, the distillation-then-RLVR training path, cheapest-component cascade, and the experiment order. |
| `deep-research/` | Ten-lane literature search run by Muse Spark 1.3 sub-agents on 2026-09-11: `REPORT.md` (verdict, top twenty, ten refutations, per-lane findings, Folio-gate security requirements, gaps), the merged 280-item inventory, and 240 admitted PDFs on disk. |
| `deep-research-4/` | Ten-lane harness-design research run by Muse Spark 1.3 sub-agents on 2026-09-13 (micro-harness architectures, ingest routing, recall, thinking modes, real-time co-processor, caching and scheduling, code graphs, learning graphs, SQL facts store, evaluation and rewards): `REPORT.md` assembled by the main session, 348 unique items, 173 admitted, 23 refutations; the design it feeds is `.planning/notes/melodyscribe-harness-frameworks.md`. |
